...
if(strlen($_SESSION['alogin'])==0){ 
  header('location:home.php');
    ...
    if (isset($_POST['update'])) { 
        ...
        $f_id = mysqli_real_escape_string($con, $_GET['f_id']);
    
        // Fetch the current status from the database
        $sql = "SELECT Status FROM feedback WHERE Id='$f_id'";
        ...
        if ($result && $row = mysqli_fetch_assoc($result)) {
            ...
            if ($newStatus !== $currentStatus) {
                $updateQuery = "UPDATE feedback SET Status='$newStatus' WHERE Id='$f_id'";
                if (mysqli_query($con, $updateQuery)) {
                    echo "<script>
                            alert('Status Updated Successfully');
                            window.location.href = '" . $_SERVER['PHP_SELF'] . "?f_id=$f_id';
                          </script>";
                    exit();
                } else {
    ...
    <div style="margin-left:50px;">
         <form name="updateticket" id="updatecomplaint" method="post"> 
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <?php 
                    $f_id = mysqli_real_escape_string($con, $_GET['f_id']);
                    $sql=mysqli_query($con, "SELECT * FROM feedback WHERE Id=$f_id"); 
                ...
                <tr height="50">
                  ...
                  <td><select name="newstatus" required="required">
                    ...
                    <option value="NotPublished">Un Published</option>
                    </select>
                  </td>
                </tr>
                <?php }?>
