...
if(strlen($_SESSION['alogin'])==0){
    ...
    if(isset($_GET['p_id'])&& $_GET['action']=='live'){ 
      $status ='Active';
      $query=mysqli_query($con,"UPDATE packages SET Status='$status' WHERE Id=$_GET[p_id]");
      header('location:packages.php');
      if($query){
        echo "<script>alert('Package is Live')</script>";
    }
  }
  elseif(isset($_GET['p_id'])&& $_GET['action']=='stp'){
    $status = 'NotActive';
    $query=mysqli_query($con,"UPDATE packages SET Status='$status' WHERE Id=$_GET[p_id]");
      header('location:packages.php');
      if($query){
        echo "<script>alert('Package is Not Atcive Now')</script>";
    }
  }
?>
<!DOCTYPE html>
                        ...
                        <div class="content">
                            <div class="module">
                                ...
                                <div class="module-body">
                                    <table id="datatable-1" class="datatable-1 table table-bordered table-striped display" width="100%">
                                        <thead>
                                            <tr>
                                                <th>#</th>
                                                <!-- <th>Destination</th> -->
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <!-- <?php 
                                                    //$des=mysqli_query($con, "SELECT DestinationName FROM destination WHERE Id=$row[Id]");
                                                ?>
                                                <td><?php // echo $des; ?></td> -->
                                                ...
                                                <td>
                                                    <?php 
                                                        if($row['Status']=="Active"){?>
                                                            ...
                                                            <a href ="packages.php?p_id=<?php echo $row['Id'];?>&&action=stp" title="Stop" onClick="return confirm('Do You Really Want To Stop ? ')"><button class="btn btn-danger btn" type="button">Stop</button><a>
                                                    <?php } else{?>
                                                       
                                                            <a href ="packages.php?p_id=<?php echo $row['Id'];?>&&action=live" title="Live" onClick="return confirm('You Want To Live ? ')"><button class="btn btn-info" type="button">Live</button></a>
                                                    <?php }?>
                                                </td>
                                            </tr>
                                            <?php } ?> 
